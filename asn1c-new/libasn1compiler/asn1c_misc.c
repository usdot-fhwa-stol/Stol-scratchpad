        ...
        if((expr->expr_type == ASN_BASIC_REAL
           ...
           || asn1c_type_fits_long(arg, expr)) {
			...
			case TNF_CONSTYPE:
				if(expr->expr_type == ASN_BASIC_REAL) {
                    return "double";
                } else if((i = asn1c_type_fits_char(arg, expr)) == FL_FITS_SIGNED) {
                    return "char";
                } else if(i == FL_FITS_UNSIGN) {
                    return "unsigned char";
                } else if((i = asn1c_type_fits_short(arg, expr)) == FL_FITS_SIGNED) {
                    return "short";
                } else if(i == FL_FITS_UNSIGN) {
...
enum asn1c_fitslong_e asn1c_type_fits_short(arg_t *arg, asn1p_expr_t *expr) {
	asn1cnst_range_t *range = 0;
	asn1cnst_edge_t left;
	asn1cnst_edge_t right;
	asn1p_expr_t *v;
 ...
 * It is defined here as a constant expression.
 */
#define	RIGHTMAX16	32767	/* of 32-bit integer type */
#define	LEFTMIN16	(-RIGHTMAX16-1)	/* of 32-bit integer type */
	...
	TQ_FOR(v, &(expr->members), next) {
		if(v->expr_type != A1TC_UNIVERVAL)
			continue;
		if(v->value->value.v_integer < LEFTMIN16
		|| v->value->value.v_integer > RIGHTMAX16)
			return FL_NOTFIT;
	}
	...
	if(range) {
		if(!range->incompatible) {
			right = range->right;
			/* Use 4 instead of sizeof(long) is justified! */
			if(right.type == ARE_VALUE && right.value <= 2)
				return FL_FITS_SIGNED;
		}
		asn1constraint_range_free(range);
	}
	...
	/* Special case for unsigned */
    if(!(arg->flags & A1C_USE_WIDE_TYPES) && left.type == ARE_VALUE
       && left.value >= 0 && left.value <= RIGHTMAX16
       && right.type == ARE_MAX) {
        return FL_FITS_UNSIGN;
    }
    if(left.type == ARE_VALUE
		&& left.value >= 0
	&& right.type == ARE_VALUE
		&& right.value > RIGHTMAX16
		&& right.value <= (asn1c_integer_t)(65535))
		return FL_FITS_UNSIGN;
		

	/* If some fixed value is outside of target range, not fit */
	if(left.type == ARE_VALUE
			&& (left.value < LEFTMIN16 || left.value > RIGHTMAX16))
		return FL_NOTFIT;
	if(right.type == ARE_VALUE
			&& (right.value > RIGHTMAX16 || right.value < LEFTMIN16))
		return FL_NOTFIT;
...
enum asn1c_fitslong_e asn1c_type_fits_char(arg_t *arg, asn1p_expr_t *expr) {
	asn1cnst_range_t *range = 0;
	asn1cnst_edge_t left;
	asn1cnst_edge_t right;
	asn1p_expr_t *v;
 ...
 * It is defined here as a constant expression.
 */
#define	RIGHTMAX8	127	/* of 8-bit integer type */
#define	LEFTMIN8	(-RIGHTMAX8-1)	/* of 8-bit integer type */
	...
	TQ_FOR(v, &(expr->members), next) {
		if(v->expr_type != A1TC_UNIVERVAL)
			continue;
		if(v->value->value.v_integer < LEFTMIN8
		|| v->value->value.v_integer > RIGHTMAX8)
			return FL_NOTFIT;
	}
	...
	if(range) {
		if(!range->incompatible) {
			right = range->right;
			/* Use 4 instead of sizeof(char) is justified! */
			if(right.type == ARE_VALUE && right.value <= 1)
				return FL_FITS_SIGNED;
		}
		asn1constraint_range_free(range);
	}
	...
	/* Special case for unsigned */
    if(!(arg->flags & A1C_USE_WIDE_TYPES) && left.type == ARE_VALUE
       && left.value >= 0 && left.value <= RIGHTMAX8
       && right.type == ARE_MAX) {
        return FL_FITS_UNSIGN;
    }
    if(left.type == ARE_VALUE
		&& left.value >= 0
	&& right.type == ARE_VALUE
		&& right.value > RIGHTMAX8
		&& right.value <= (asn1c_integer_t)(255))
		return FL_FITS_UNSIGN;
		

	/* If some fixed value is outside of target range, not fit */
	if(left.type == ARE_VALUE
			&& (left.value < LEFTMIN8 || left.value > RIGHTMAX8))
		return FL_NOTFIT;
	if(right.type == ARE_VALUE
			&& (right.value > RIGHTMAX16 || right.value < LEFTMIN16))
		return FL_NOTFIT;
